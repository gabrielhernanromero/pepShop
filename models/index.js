...
const Product = require('./Product'); * Modelos Sequelize - Definición de entidades y asociaciones

const Cliente = require('./Cliente'); * Este archivo configura la conexión a la base de datos SQLite y define

const Mascota = require('./Mascota'); * todos los modelos (tablas) con sus campos y relaciones.
...
// Definir asociaciones entre modelosrequire('dotenv').config();

Cliente.hasMany(Mascota, { foreignKey: 'clienteId', as: 'mascotas' });

Mascota.belongsTo(Cliente, { foreignKey: 'clienteId', as: 'propietario' });// Obtener la ruta del archivo SQLite desde .env (DATABASE_URL="file:./dev.db")
...
Cliente.hasMany(Pedido, { foreignKey: 'clienteId', as: 'pedidos' }); * - dialect: 'sqlite' - Usa base de datos SQLite (ideal para desarrollo)

Pedido.belongsTo(Cliente, { foreignKey: 'clienteId', as: 'cliente' }); * - storage: ruta al archivo .db

 * - logging: false - No mostrar queries SQL en consola (cambiar a true para debug)

// Exportar todos los modelos */

module.exports = {const sequelize = new Sequelize({

  Product,  dialect: 'sqlite',

  Cliente,  storage: dbFile,

  Mascota,  logging: false,
